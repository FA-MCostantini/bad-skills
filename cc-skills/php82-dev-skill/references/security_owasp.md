...
```php
// NEVER use exec/system with user input directly
// If shell commands are necessary:
$safeArg = escapeshellarg($userInput);
$output = shell_exec("command {$safeArg}");

// Prefer: use PHP libraries instead of shell commands
```
...
// If unavoidable, restrict allowed classes:
